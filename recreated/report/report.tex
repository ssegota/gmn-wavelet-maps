...
detected peak giving its position, speed, significance, the number of meteors
that formed it, the mean orbit, and the associated shower name (for example
``EVI eta Virginids''). The maxlist lists \emph{every} significant peak, not only
catalogued showers: peaks with no catalogue match are included as well, and the
\flag{--mark-unknown} option flags them as shower candidates --- both as a
\texttt{UNK} tag in the table and as white ``?'' circles on the maps.
...
epoch to the next, exercising the full pipeline on independent data. These maps
were generated with \flag{--mark-unknown}, so significant peaks with no catalogue
match are circled in white with a ``?'' as shower \emph{candidates} (and tagged
\texttt{UNK} in the maxlists); the named coloured circles are catalogue
associations. Most candidates are minor known showers absent from the public
catalogue or sporadic-source fluctuations, but the flag makes any genuinely new
concentration easy to spot.
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    \flag{--mark-unknown}              & off    & flag unmatched peaks as candidates (UNK + ``?'')\\
    \flag{--candidate-min-xsig}        & 8.0    & min xsig to draw a candidate on the maps\\
...
\paragraph{Testing on real data.} All testing uses real GMN data. A fast smoke
test is a single daily file in \texttt{self} mode (no yearly data needed), which
produces both maps and a maxlist in seconds:
\begin{lstlisting}
python gmn_wavelet_maps.py --date 20250318      # or --file <daily.txt>
\end{lstlisting}
The full end-to-end check is the \texttt{annual} run on real radiants compared
against GMN's real published product: the maxlist is compared with GMN's
\texttt{original/example.txt} (positions and \texttt{xsig} of the strong showers,
Section~\ref{sec:same}) and the rendered SCE map with the published PNG. Two
further quantitative cross-checks confirm correctness on real data: the recovered
shower radiants match the published catalogue radiants to $\lesssim 1^\circ$
across six independent epochs (Section~\ref{sec:diff}), and the post-quality
radiant count reproduces GMN's header value (NOrb${}=2585$ vs.\ $2574$). Running
any other date is then a one-line change, e.g.:
\begin{lstlisting}
python gmn_wavelet_maps.py --date 20251214 \
       --background annual --bg-file traj_summary_yearly_2025.txt \
       --quality --bg-cache bg_2025.npz --out out
